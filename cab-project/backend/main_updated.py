...
def request_ride(ride_data: RideRequest, user_id: int = Depends(verify_token), db: Session = Depends(get_db)):
    ...
    ride = Ride(
        rider_id=user_id,
        pickup_point=ST_Point(ride_data.pickup_lng, ride_data.pickup_lat),
        drop_point=ST_Point(ride_data.drop_lng, ride_data.drop_lat),
    ...
    try:
        while True:
            ...
            if message.get("type") == "location_update":
                if driver:
                    driver.current_location = ST_Point(message["lng"], message["lat"])
                    db.commit()
